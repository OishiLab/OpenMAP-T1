...
def create_parcellated_images(output, output_dir, basename, odata, data):
    ...
    # Read the CSV files (df_name is loaded for possible future use)
    df_no = pd.read_csv('../level/Level_ROI_No.csv')
    df_name = pd.read_csv('../level/Level_ROI_Name.csv')
    ...
    for level in all_level:
        ...
        os.makedirs(os.path.join(output_dir, f"parcellated"), exist_ok=True)
